...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("alina_cart");
      if (saved) setItems(JSON.parse(saved));
      const savedDirect = localStorage.getItem("alina_direct_buy");
  ...
  useEffect(() => {
    try {
      localStorage.setItem("alina_cart", JSON.stringify(items));
    } catch (e) {
  ...
  const clearCart = () => {
    ...
    localStorage.removeItem("alina_cart");
    localStorage.removeItem("alina_direct_buy");
  };
  ...
  const buyNow = (item: Omit<CartItem, "id">) => {
    ...
    try {
      localStorage.setItem("alina_direct_buy", JSON.stringify(cartItem));
    } catch (e) {}
